(function($){
	...
	$("#menu form").submit(function () {
		...
		$.ajax({
			...
			complete: function (data, status) {
				try{
					window.history.pushState(tabela, document.title, link);
				}
				catch(e){
					console.log(e);
				}
	...
	function ajaxTorrentsPage() {
		ajaxBegin($("#tabela1"));
		ajaxStop($("#tabela1"));		
		var todosLinks = $.grep(tabela.siblings().find("a"), function(item){
			var curItem = $(item)
			if(curItem.attr("href").indexOf("page") != -1)
			return curItem;
		});
		$.each(todosLinks, function(i, item) {
			$(item).click(function () {
